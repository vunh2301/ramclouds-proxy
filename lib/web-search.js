...
const quotaCounters = {}; // { backend: { month: "2026-03", count: 0 } }
...
function incrementQuota(backend) {
  ...
  const limit = QUOTA_LIMITS[backend];
  if (limit && quotaCounters[backend].count % 100 === 0) {
    console.log(`[web-search/quota] ${backend}: ${quotaCounters[backend].count}/${limit} this month`);
  }
}
