...
// ==================== Rate limiter (monthly quota, persisted to file) ====================
const fs = require("fs");
const path = require("path");

const QUOTA_FILE = path.join(__dirname, "..", "search-quota.json");
const QUOTA_LIMITS = {
  brave:  parseInt(process.env.BRAVE_MONTHLY_LIMIT  || "2000", 10),
  tavily: parseInt(process.env.TAVILY_MONTHLY_LIMIT || "1000", 10),
  exa:    parseInt(process.env.EXA_MONTHLY_LIMIT    || "1000", 10),
  serper: parseInt(process.env.SERPER_MONTHLY_LIMIT || "2500", 10),
};
...
try {
  ...
  quotaCounters = JSON.parse(raw);
  console.log("[web-search/quota] loaded from", QUOTA_FILE);
...
function saveQuota() {
  try { fs.writeFileSync(QUOTA_FILE, JSON.stringify(quotaCounters, null, 2)); } catch {}
}

function checkQuota(backend) {
  const limit = QUOTA_LIMITS[backend];
  if (!limit) return true;
  const month = getCurrentMonth();
  if (!quotaCounters[backend] || quotaCounters[backend].month !== month) {
    quotaCounters[backend] = { month, count: 0 };
  }
  return quotaCounters[backend].count < limit;
}

function incrementQuota(backend) {
  const month = getCurrentMonth();
  if (!quotaCounters[backend] || quotaCounters[backend].month !== month) {
    quotaCounters[backend] = { month, count: 0 };
  }
  quotaCounters[backend].count++;
  const limit = QUOTA_LIMITS[backend];
  if (limit) {
    const c = quotaCounters[backend].count;
    if (c % 50 === 0 || c === limit) {
      console.log(`[web-search/quota] ${backend}: ${c}/${limit} this month`);
    }
  }
  saveQuota();
}
...
function getQuotaStatus() {
  const month = getCurrentMonth();
  const status = {};
  for (const [backend, limit] of Object.entries(QUOTA_LIMITS)) {
    const c = quotaCounters[backend];
    status[backend] = { used: (c && c.month === month) ? c.count : 0, limit };
  }
  return status;
}
...
/**
 * Exa AI Search — neural/semantic search designed for AI agents.
 * Free 1000 searches/month, no credit card needed.
 */
async function searchExa(query, count) {
  ...
  try {
    ...
    if (!res.ok) {
      console.log("[web-search/exa] error:", res.status);
      return null;
    }
    const data = await res.json();
    const results = (data.results || []).slice(0, count);
    ...
    return results.map((r, i) =>
      `[${i + 1}] ${r.title}\n    ${r.url}\n    ${r.text || r.snippet || "(no description)"}`
    ).join("\n\n");
  } catch (e) {
    console.log("[web-search/exa] fetch error:", e?.message);
    return null;
  }
}
...
const FALLBACK_CHAINS = {
  brave:      [searchBrave, searchTavily, searchExa, searchSerper, searchGoogleNews, searchDuckDuckGoHTML, searchBing, searchDuckDuckGo, searchSearXNG],
  tavily:     [searchTavily, searchExa, searchSerper, searchGoogleNews, searchDuckDuckGoHTML, searchBing, searchDuckDuckGo, searchSearXNG],
  exa:        [searchExa, searchTavily, searchSerper, searchGoogleNews, searchDuckDuckGoHTML, searchBing, searchDuckDuckGo, searchSearXNG],
  serper:     [searchSerper, searchTavily, searchExa, searchGoogleNews, searchDuckDuckGoHTML, searchBing, searchDuckDuckGo, searchSearXNG],
...
// Map function → backend name for quota tracking
const FN_TO_BACKEND = new Map([
  [searchBrave, "brave"], [searchTavily, "tavily"], [searchExa, "exa"], [searchSerper, "serper"],
]);

// Backend display info: emoji + name
const BACKEND_TAGS = {
  brave:    { icon: "🦁", name: "Brave" },
  tavily:   { icon: "🔮", name: "Tavily" },
  exa:      { icon: "🧠", name: "Exa" },
  serper:   { icon: "🔍", name: "Serper" },
  gnews:    { icon: "📰", name: "Google News" },
  ddghtml:  { icon: "🦆", name: "DuckDuckGo" },
  bing:     { icon: "🅱️", name: "Bing" },
  ddglite:  { icon: "🦆", name: "DDG Lite" },
  searxng:  { icon: "🔎", name: "SearXNG" },
};

// Map function → backend tag key
const FN_TO_TAG = new Map([
  [searchBrave, "brave"], [searchTavily, "tavily"], [searchExa, "exa"],
  [searchSerper, "serper"], [searchGoogleNews, "gnews"],
  [searchDuckDuckGoHTML, "ddghtml"], [searchBing, "bing"],
  [searchDuckDuckGo, "ddglite"], [searchSearXNG, "searxng"],
]);

/**
 * Build search tag banner for display.
 */
function buildSearchTag(tagKey) {
  const tag = BACKEND_TAGS[tagKey] || { icon: "🔍", name: tagKey };
  const quotaKey = FN_TO_BACKEND.get([...FN_TO_TAG.entries()].find(([, v]) => v === tagKey)?.[0]);
  const limit = QUOTA_LIMITS[quotaKey];
  if (limit) {
    const month = getCurrentMonth();
    const c = quotaCounters[quotaKey];
    const used = (c && c.month === month) ? c.count : 0;
    return `${tag.icon} **${tag.name}** (${used}/${limit})`;
  }
  return `${tag.icon} **${tag.name}**`;
}

/**
 * Search using configured backend with automatic fallback + quota enforcement.
 * Returns { text, tag } where tag is a display string like "🔮 **Tavily** (5/1000)"
 */
async function search(query, count) {
  if (!query) return { text: "[Empty search query]", tag: "" };
  ...
  for (const fn of chain) {
    const backend = FN_TO_BACKEND.get(fn);
    if (backend && !checkQuota(backend)) {
      console.log(`[web-search] ${backend} quota exceeded, skipping`);
      continue;
    }
    const result = await fn(query, count);
    if (result) {
      if (backend) incrementQuota(backend);
      const tagKey = FN_TO_TAG.get(fn) || "unknown";
      const tag = buildSearchTag(tagKey);
      return { text: result, tag };
    }
  }

  return { text: "[No search results found]", tag: "❌ No backend" };
}
