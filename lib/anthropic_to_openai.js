...
async function pipeResponsesStreamAsAnthropic(upRes, res, requestModel) {
    ...
    while (true) {
      ...
      for (const part of parts) {
        ...
        try { data = JSON.parse(dataStr); } catch (e) {
          console.log("[responses→anthropic] JSON parse error:", e.message, "data:", dataStr.slice(0, 100));
          continue;
        }

        // Log event types for debugging (only non-delta events to avoid spam)
        if (eventType && !eventType.includes(".delta")) {
          console.log(`[responses→anthropic] event: ${eventType}`);
        }

        // Error event — log details and inject error text into stream
        if (eventType === "error") {
          const errMsg = data.error?.message || data.message || JSON.stringify(data).slice(0, 300);
          const errCode = data.error?.code || data.code || "unknown";
          console.log(`[responses→anthropic] ERROR: code=${errCode} msg=${errMsg}`);
          // Inject error as text so user sees something
          if (!textBlockOpen) {
            res.write(`event: content_block_start\ndata: ${JSON.stringify({
              type: "content_block_start", index: blockIndex,
              content_block: { type: "text", text: "" },
          ...
          res.write(`event: content_block_delta\ndata: ${JSON.stringify({
            type: "content_block_delta", index: blockIndex,
            delta: { type: "text_delta", text: `\n\n[OpenAI Error: ${errCode} — ${errMsg}]` },
          })}\n\n`);
          continue;
        }

        // Response failed — log and continue (will close stream normally)
        if (eventType === "response.failed") {
          const status = data.response?.status || "failed";
          const errDetail = data.response?.error || data.response?.incomplete_details || {};
          console.log(`[responses→anthropic] FAILED: status=${status} detail=${JSON.stringify(errDetail).slice(0, 200)}`);
          continue;
        }
  ...
  console.log(`[responses→anthropic] done: ${blockIndex} blocks, ${hasToolCalls ? "tool_use" : "text"}, usage: in=${usage.input_tokens||0} out=${usage.output_tokens||0}`);
