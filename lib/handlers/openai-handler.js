...
async function handleOpenAI(req, res, incoming, stream, config) {
    ...
    if (hasWebSearch) {
        console.log("[openai] web_search_preview detected → converted to function tool");
    ...
    if (hasResponsesFormat) {
        ...
        console.log("[responses] -> responses pass-through", responsesUrl, "input=", Array.isArray(incoming.input) ? incoming.input.length : 0, "tools=", enrichedTools?.length || 0, "webSearch=", hasWebSearch);
        ...
        if (stream && upRes.body) {
            ...
            if (interceptedCalls.length > 0) {
                await handleWebSearchFollowUp(res, body, interceptedCalls[0], responsesUrl, apiKey, chatId, incoming.model);
    ...
    console.log("[openai] incoming keys:", Object.keys(incoming).join(", "), "has messages=", !!incoming.messages, "has input=", !!incoming.input, "webSearch=", hasWebSearch);
    ...
    if (stream) {
        ...
        // Session commit (use non-intercepted data)
        const msg = { role: "assistant", content: assistantText || null };
        const nonInterceptedTools = assistantToolCalls.filter(tc => tc.function?.name !== "web_search");
        if (nonInterceptedTools.length > 0) msg.tool_calls = nonInterceptedTools;
...
/**
 * Execute web search and stream follow-up response.
 */
async function handleWebSearchFollowUp(res, originalBody, wsCall, responsesUrl, apiKey, chatId, model) {
    const query = extractQuery(wsCall);
    console.log("[web-search] intercepted, query:", JSON.stringify(query));

    const { text: searchResults, tag: searchTag } = await webSearch(query);
    console.log("[web-search] results:", (searchResults || "").length, "chars", searchTag);

    const followUpBody = buildWebSearchFollowUp(originalBody, wsCall, searchResults);
    followUpBody.stream = true;

    console.log("[web-search] -> follow-up request");
    ...
    if (followUpRes.ok && followUpRes.body) {
        ...
        console.log("[web-search] follow-up ERROR:", followUpRes.status, errText.slice(0, 200));
        emitStreamFinish(res, chatId, model);
    }
}
