...
function createAmpProviderRouter(config, { handleClaude, handleOpenAI, handleAmp, resolveModel, modelMap, defaultModel, isAnthropicModel, anthropicPrefixes, isAmpRequest, ampPrefixes, buildAnthropicHeaders, pipeAnthropicStream, collectStreamToResponse, setContentTypeFromUpstream }) {
  ...
  router.post("/anthropic/v1/messages", async (req, res) => {
    ...
    // --- Web search detection: use Ramclouds native Responses API ---
    const hasWebSearch = Array.isArray(body.tools) && body.tools.some(t => {
      const ttype = t?.type || "";
      const tname = t?.name || "";
      return ttype === "web_search_20250305" || ttype === "web_search" ||
    ...
    if (hasWebSearch) {
      // Convert Anthropic body → Responses API, forward to Ramclouds /v1/responses with native web_search
      const ramcloudsResponsesUrl = (config.PROVIDER_BASE_URL || "https://ramclouds.me/v1").replace(/\/+$/, "").replace(/\/messages$/, "") + "/responses";
      const respBody = anthropicBodyToResponsesAPI(body);
      respBody.model = body.model;
      // Replace any web_search variants with the native type Ramclouds supports
      respBody.tools = (respBody.tools || []).filter(t => {
        const ttype = t?.type || "";
        return !ttype.startsWith("web_search") && ttype !== "web_search";
      });
      respBody.tools.push({ type: "web_search" });
      respBody.stream = clientWantsStream;
      const apiKey = config.PROVIDER_API_KEY || "";
      console.log(`[amp/anthropic] web_search -> Ramclouds Responses API ${ramcloudsResponsesUrl} model=${body.model}`);
      try {
        const upRes = await fetch(ramcloudsResponsesUrl, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        if (upRes.ok) {
          console.log(`[amp/anthropic] web_search native: status=200 stream=${clientWantsStream}`);
          if (clientWantsStream) {
            return await pipeResponsesStreamAsAnthropic(upRes, res, body.model);
          }
          const data = await upRes.json();
          const assembled = openAIResponseToAnthropic(data, body.model);
          return res.json(assembled);
        }
        const errText = await upRes.text().catch(() => "");
        console.log(`[amp/anthropic] web_search native failed: ${upRes.status} ${errText.slice(0, 200)} — falling back to DIY search`);
      } catch (e) {
        console.log("[amp/anthropic] web_search native error:", e?.message, "— falling back to DIY search");
      }
      // Fallback: DIY search (old path) — replace web_search tool with function tool
      body.tools = body.tools.filter(t => {
        const ttype = t?.type || "";
        const tname = t?.name || "";
        return ttype !== "web_search_20250305" && ttype !== "web_search" &&
      ...
      console.log("[amp/anthropic] web_search fallback -> DIY function tool");
    }
