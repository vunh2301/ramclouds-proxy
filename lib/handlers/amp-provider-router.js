...
const { anthropicBodyToOpenAI, openAIResponseToAnthropic, pipeOpenAIStreamAsAnthropic, anthropicBodyToResponsesAPI, pipeResponsesStreamAsAnthropic } = require("../anthropic_to_openai");
...
function createAmpProviderRouter(config, { handleClaude, handleOpenAI, handleAmp, resolveModel, modelMap, defaultModel, isAnthropicModel, anthropicPrefixes, isAmpRequest, ampPrefixes, buildAnthropicHeaders, pipeAnthropicStream, collectStreamToResponse, setContentTypeFromUpstream }) {
  ...
  function responsesInputToMessages(input) {
    ...
    let system = null;
    const messages = [];
    // Track generated IDs for function_call → function_call_output matching
    let lastGeneratedCallId = null;
    for (const item of input) {
      ...
      if (item.type === "function_call") {
        ...
        const callId = item.call_id || item.id || ("toolu_" + Date.now().toString(36) + Math.random().toString(36).slice(2, 6));
        lastGeneratedCallId = callId;
        messages.push({
          role: "assistant",
          content: [{
            type: "tool_use",
      ...
      if (item.type === "function_call_output") {
        // Use call_id from item, or fall back to the last function_call's generated ID
        const toolUseId = item.call_id || item.id || lastGeneratedCallId || ("toolu_" + Date.now().toString(36) + Math.random().toString(36).slice(2, 6));
        lastGeneratedCallId = null;
        messages.push({
          role: "user",
          content: [{
            type: "tool_result",
  ...
  // ---- OpenAI direct mode: forward Responses API to OpenAI without conversion ----
  const providerMode = config.PROVIDER_MODE || "ramclouds";
  const openaiDirectUrl = (config.OPENAI_BASE_URL || "https://api.openai.com/v1").replace(/\/+$/, "");
  const openaiDirectKey = config.OPENAI_API_KEY || "";
  const openaiModelMap = config.OPENAI_MODEL_MAP || {};

  // Resolve ramclouds model name → real OpenAI model name (e.g. gpt-5.4 → gpt-4o)
  function resolveOpenAIModel(model) {
    return openaiModelMap[model] || model;
  }

  function shouldUseOpenAIDirect(model) {
    if (providerMode === "openai") return true;
    if (providerMode === "auto" && model && /^(gpt-|o[1-9]|chatgpt-)/.test(model)) return true;
    return false;
  }

  if (providerMode !== "ramclouds") {
    console.log(`[provider] mode=${providerMode} openai=${openaiDirectUrl}`);
  }

  // OpenAI responses API → convert to Messages API on ramclouds (or direct to OpenAI)
  ...
  router.post("/openai/v1/responses", async (req, res) => {
    ...
    // ---- OpenAI direct mode: forward as-is to OpenAI ----
    if (shouldUseOpenAIDirect(incomingModel) && openaiDirectKey) {
      const ampResolved = resolveAmpModel(incomingModel);
      const resolvedModel = resolveOpenAIModel(ampResolved);
      // Keep web_search_preview — OpenAI handles it natively
      console.log(`[amp/responses/openai-direct] ${incomingModel} -> ${ampResolved} -> ${resolvedModel} (native OpenAI)`);
      body.model = resolvedModel;

      try {
        const upRes = await fetch(openaiDirectUrl + "/responses", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": "Bearer " + openaiDirectKey,
        ...
        console.log(`[amp/responses/openai-direct] status=${upRes.status}`);
        if (!upRes.ok) {
          const t = await upRes.text().catch(() => "");
          console.log("[amp/responses/openai-direct] ERROR:", t.slice(0, 300));
          return res.status(upRes.status).send(t || "Error " + upRes.status);
        }

        // Stream or pipe response directly
        ...
        console.log("[amp/responses/openai-direct] ERROR:", e?.message);
        if (res.headersSent) return res.end();
        return res.status(500).json({ error: { message: e?.message, type: "proxy_error" } });
      }
    }
    ...
    // ---- Non-Claude models: pass-through to ramclouds /v1/responses with reasoning ----
    if (!isAnthropicModel(startModel, anthropicPrefixes)) {
      body.model = startModel;
      // Inject reasoning effort
      if (config.REASONING_EFFORT && config.REASONING_EFFORT !== "none") {
        body.reasoning = { effort: config.REASONING_EFFORT };
        console.log(`[amp/responses] injected reasoning: effort=${config.REASONING_EFFORT}`);
      }
      // Filter unsupported fields
      const unsupported = ["metadata", "store", "include", "prompt_cache_retention", "stream_options"];
      unsupported.forEach(k => delete body[k]);

      const responsesUrl = config.providerEndpoint("/responses");
      console.log(`[amp/responses] -> ${responsesUrl} model=${startModel} stream=${clientStream} (pass-through)`);
      ...
      try {
        ...
        console.log(`[amp/responses] ${role ? role + ": " : ""}${startModel} status=${upRes.status} stream=${clientStream} (pass-through)`);
        if (!upRes.ok) {
          const t = await upRes.text().catch(() => "");
          console.log("[amp/responses] pass-through ERROR:", t.slice(0, 300));
          return res.status(upRes.status).send(t || "Error " + upRes.status);
        }

        if (clientStream && upRes.body) {
          res.setHeader("Content-Type", "text/event-stream; charset=utf-8");
          res.setHeader("Cache-Control", "no-cache, no-transform");
          res.setHeader("Connection", "keep-alive");
          const reader = upRes.body.getReader();
        ...
        console.log("[amp/responses] pass-through ERROR:", e?.message);
        if (res.headersSent) return res.end();
        return res.status(500).json({ error: { message: e?.message, type: "proxy_error" } });
      }
    }

    // ---- Claude models: convert to Anthropic Messages API ----
    ...
    // Inject thinking for Claude models on ramclouds
    if (config.THINKING_BUDGET > 0) {
      msgBody.thinking = { type: "enabled", budget_tokens: config.THINKING_BUDGET };
      if (msgBody.max_tokens <= config.THINKING_BUDGET) {
        msgBody.max_tokens = config.THINKING_BUDGET + 8192;
      }
      console.log(`[amp/responses] injected thinking: budget=${config.THINKING_BUDGET} max_tokens=${msgBody.max_tokens}`);
    }
    ...
    try {
      ...
      if (hasWebSearch) {
        ...
        if (wsBlocks.length > 0) {
          ...
          const { text: searchResults, tag: searchTag } = await webSearch(query);
          console.log("[amp/responses] search results:", (searchResults || "").length, "chars", searchTag);
          ...
          // Remove ALL tools — force pure text answer, no tool calls that AMP client might fail to execute
          delete followUpBody.tools;
          delete followUpBody.tool_choice;
          // Add system instruction to use the search results
          const searchInstruction = "\n\nYou just performed web searches and received real, up-to-date results. IMPORTANT: Synthesize results into a well-structured answer with headings and bullet points. Mention source names but do NOT include raw URLs. Include specific facts, dates, names. Write in the same language the user used. Do NOT say you cannot search.";
  ...
  async function pipeAnthropicStreamWithBanner(upRes, res, bannerText) {
    ...
    const blockTypes = []; // track block types for logging
    ...
    function processFrame(frame) {
      ...
      if (eventType === "message_stop" || eventType === "message_delta") {
        if (blockTypes.length > 0) {
          console.log(`[amp/anthropic] stream blocks: [${blockTypes.join(",")}]`);
        }
      }
    ...
    try {
      ...
      if (blockTypes.length > 0) {
        console.log(`[amp/anthropic] stream done: ${blockTypes.length} blocks [${blockTypes.join(",")}]`);
      }
      res.end();
    }
  }
  ...
  router.post("/anthropic/v1/messages", async (req, res) => {
    ...
    // Ensure thinking is enabled for Claude models on ramclouds
    // GPT models use reasoning via Responses API path instead
    if (!body.thinking && config.THINKING_BUDGET > 0 && isAnthropicModel(startModel, anthropicPrefixes)) {
      body.thinking = { type: "enabled", budget_tokens: config.THINKING_BUDGET };
      // Ensure max_tokens > thinking budget
      if (!body.max_tokens || body.max_tokens <= config.THINKING_BUDGET) {
        body.max_tokens = config.THINKING_BUDGET + 8192;
      }
      console.log(`[amp/anthropic] injected thinking: budget=${config.THINKING_BUDGET} max_tokens=${body.max_tokens}`);
    }

    // ---- OpenAI direct mode: convert Anthropic → OpenAI Chat Completions ----
    if (shouldUseOpenAIDirect(startModel) && openaiDirectKey) {
      const openaiModel = resolveOpenAIModel(startModel);
      console.log(`[amp/anthropic/openai-direct] ${incomingModel} -> ${startModel} -> ${openaiModel} (via OpenAI Chat API)`);

      // Keep web_search tools as OpenAI hosted tools
      const hasWsNative = Array.isArray(body.tools) && body.tools.some(t =>
        (t?.type || "").startsWith("web_search") || (t?.name || "") === "web_search"
      );
      // Convert Anthropic native web_search tool types to OpenAI hosted tool
      if (hasWsNative) {
      ...
      // When web_search is needed, use Responses API (Chat Completions doesn't support hosted tools)
      if (hasWsNative) {
        console.log("[amp/anthropic/openai-direct] web_search detected → using Responses API");
        const respBody = anthropicBodyToResponsesAPI(body);
        respBody.model = openaiModel;
        // Add web_search_preview as hosted tool
        if (!respBody.tools) respBody.tools = [];
        respBody.tools.push({ type: "web_search_preview" });
        respBody.stream = clientWantsStream;
        ...
        try {
          ...
          if (!upRes.ok) {
            const t = await upRes.text().catch(() => "");
            console.log("[amp/anthropic/openai-direct/responses] ERROR:", upRes.status, t.slice(0, 300));
            console.log("[amp/anthropic/openai-direct/responses] falling back to ramclouds...");
          } else {
            console.log(`[amp/anthropic/openai-direct/responses] status=200 stream=${clientWantsStream}`);
            if (clientWantsStream) {
              return await pipeResponsesStreamAsAnthropic(upRes, res, startModel);
            }
            // Non-stream: collect full response
            const data = await upRes.json();
            const assembled = openAIResponseToAnthropic(data, startModel);
            return res.json(assembled);
          }
        } catch (e) {
          console.log("[amp/anthropic/openai-direct/responses] ERROR:", e?.message, "- falling back to ramclouds");
        }
      } else {
        // No web search → use Chat Completions API (faster, simpler)
        const openaiBody = anthropicBodyToOpenAI(body);
        openaiBody.model = openaiModel;
        openaiBody.stream = clientWantsStream;
        if (clientWantsStream) openaiBody.stream_options = { include_usage: true };

        try {
          const upRes = await fetch(openaiDirectUrl + "/chat/completions", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "Authorization": "Bearer " + openaiDirectKey,
          ...
          if (!upRes.ok) {
            const t = await upRes.text().catch(() => "");
            console.log("[amp/anthropic/openai-direct] ERROR:", upRes.status, t.slice(0, 300));
            console.log("[amp/anthropic/openai-direct] falling back to ramclouds...");
          } else {
            console.log(`[amp/anthropic/openai-direct] status=200 stream=${clientWantsStream}`);
            if (clientWantsStream) {
              return await pipeOpenAIStreamAsAnthropic(upRes, res, startModel);
            }
            const data = await upRes.json();
            const assembled = openAIResponseToAnthropic(data, startModel);
            return res.json(assembled);
          ...
          console.log("[amp/anthropic/openai-direct] ERROR:", e?.message, "- falling back to ramclouds");
        }
      }
    }

    // --- Web search detection (Anthropic format) ---
    // Ramclouds doesn't support /v1/responses, so go straight to DIY search (Brave/DDG)
    ...
    if (hasWebSearch) {
      // Replace web_search tool with function tool for DIY search
      body.tools = body.tools.filter(t => {
        const ttype = t?.type || "";
        const tname = t?.name || "";
        return ttype !== "web_search_20250305" && ttype !== "web_search" &&
      ...
      if (hasWebSearch) {
        ...
        if (wsBlocks.length > 0) {
          ...
          for (const ws of wsBlocks) {
            ...
            const { text: searchResult, tag: searchTag } = await webSearch(query, 8);
            if (searchTag && !searchTags.includes(searchTag)) searchTags.push(searchTag);
            toolResults.push({
              type: "tool_result",
              tool_use_id: ws.id,
          ...
          // Remove ALL tools from follow-up — force pure text response, no tool calls
          delete followUpBody.tools;
          delete followUpBody.tool_choice;

          // Add system instruction to use search results
          const wsSystemMsg = "You just performed web searches and received real, up-to-date results above. IMPORTANT RULES:\n1. The search results are REAL and CURRENT. Do NOT say you cannot search or that results are limited.\n2. Synthesize the results into a well-structured answer with clear headings and bullet points.\n3. Mention source names (e.g. Reuters, TechCrunch) but do NOT include raw URLs in your response — they are ugly redirect links.\n4. Include specific facts, dates, and names from the results.\n5. Write in the same language the user used.";
          ...
          // Build search banner: "🔮 Tavily (5/1000) · 3 queries"
          const searchBanner = searchTags.length > 0
            ? `${searchTags.join(" · ")} · ${wsBlocks.length} ${wsBlocks.length === 1 ? "query" : "queries"}`
            : "";
          const combinedBanner = [banner, searchBanner].filter(Boolean).join("\n");

          try {
            const followUp = await tryMessagesAPI(followUpBody, headers);
            if (followUp.ok) {
              if (clientWantsStream) {
                return await pipeAnthropicStreamWithBanner(followUp.upRes, res, combinedBanner);
              }
              const followAssembled = await collectStreamToResponse(followUp.upRes);
              if (combinedBanner) followAssembled.content.unshift({ type: "text", text: combinedBanner + "\n\n" });
        ...
        if (clientWantsStream) {
          ...
          res.setHeader("Cache-Control", "no-cache, no-transform");
          res.setHeader("Connection", "keep-alive");
          const responseId = assembled.id || ("msg_" + Date.now().toString(36));
          const usage = assembled.usage || { input_tokens: 0, output_tokens: 0, cache_creation_input_tokens: 0, cache_read_input_tokens: 0 };
          res.write(`event: message_start\ndata: ${JSON.stringify({ type: "message_start", message: { id: responseId, type: "message", role: "assistant", model: body.model, content: [], stop_reason: null, usage: { input_tokens: usage.input_tokens || 0, output_tokens: 0 } } })}\n\n`);
          ...
          res.write(`event: message_delta\ndata: ${JSON.stringify({ type: "message_delta", delta: { stop_reason: assembled.stop_reason || "end_turn" }, usage: { output_tokens: usage.output_tokens || 0 } })}\n\n`);
          res.write(`event: message_stop\ndata: ${JSON.stringify({ type: "message_stop" })}\n\n`);
          return res.end();
        }
        console.log(`[amp/anthropic] assembled: ${assembled.content.length} blocks [${assembled.content.map(b=>b.type).join(",")}]`);
        return res.json(assembled);
      }
