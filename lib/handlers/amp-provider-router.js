...
function createAmpProviderRouter(config, { handleClaude, handleOpenAI, handleAmp, resolveModel, modelMap, defaultModel, isAnthropicModel, anthropicPrefixes, isAmpRequest, ampPrefixes, buildAnthropicHeaders, pipeAnthropicStream, collectStreamToResponse, setContentTypeFromUpstream }) {
  ...
  const MODEL_PROBE_INTERVAL_MS = parseInt(process.env.AMP_MODEL_PROBE_INTERVAL_MS, 10) || 30000; // 30s between probes
  const downModels = new Map(); // model -> { until, probeTimer }

  function markModelDown(model) {
    const existing = downModels.get(model);
    if (existing?.probeTimer) return; // already down with probe running
    const entry = { until: Date.now() + MODEL_DOWN_TTL_MS, probeTimer: null };
    downModels.set(model, entry);
  ...
  // Background probe: send minimal request to check if model is back
  function scheduleProbe(model) {
    const entry = downModels.get(model);
    if (!entry) return;

    entry.probeTimer = setTimeout(async () => {
      if (!downModels.has(model)) return; // already recovered
      ...
      try {
        ...
        if (upRes.ok || !RETRY_STATUSES.has(upRes.status)) {
          // Model is back (or got a non-retryable error like 400 — still means it's responding)
          await upRes.text().catch(() => ""); // drain body
          markModelUp(model);
        } else {
          const t = await upRes.text().catch(() => "");
          console.log(`[amp/probe] ${model} still DOWN (${upRes.status})`);
          // Extend TTL and schedule next probe
          const entry2 = downModels.get(model);
          if (entry2) {
            entry2.until = Date.now() + MODEL_DOWN_TTL_MS;
            scheduleProbe(model);
          }
        }
      } catch (e) {
        console.log(`[amp/probe] ${model} probe error: ${e?.message}`);
        scheduleProbe(model); // retry later
      }
    }, MODEL_PROBE_INTERVAL_MS);

    // Don't block process exit
    if (entry.probeTimer?.unref) entry.probeTimer.unref();
  }
  ...
  router.post("/openai/v1/responses", async (req, res) => {
    ...
    const resolved = resolveAmpModel(incomingModel);
    if (resolved !== incomingModel) {
      console.log("[amp/provider/openai/responses] model:", incomingModel, "->", resolved);
    }
    ...
    console.log("[amp/provider/openai/responses] ->", config.providerEndpoint("/messages"), "model=", resolved, "stream=", clientStream, "msgs=", messages.length);

    const headers = buildAnthropicHeaders(req, config);

    try {
      const result = await tryMessagesAPI(msgBody, headers);

      console.log("[amp/provider/openai/responses] status:", result.ok ? 200 : result.status);

      if (!result.ok) {
        console.log("[amp/provider/openai/responses] ERROR:", (result.errorText || "").slice(0, 300));
        if (res.headersSent) return res.end();
        return res.status(result.status).send(result.errorText || "Error " + result.status);
      }

      if (clientStream) {
        return await pipeMessagesAsResponsesStream(result.upRes, res, resolved);
      }
      ...
      console.log("[amp/provider/openai/responses] ERROR:", e?.message || e);
      if (res.headersSent) return res.end();
      return res.status(500).json({ error: { message: e?.message || String(e), type: "proxy_runtime_error" } });
    }
  });
  ...
  router.post("/anthropic/v1/messages", async (req, res) => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        if (res.headersSent) return res.end();
        setContentTypeFromUpstream(result.upRes, res);
        return res.status(result.status).send(result.errorText || "Error " + result.status);
      }
