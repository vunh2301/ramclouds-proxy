...
function resolveAmpBun() {
  // Resolve amp CLI to bun/amp binary directly
  // Mac: ~/.amp/bin/amp is the bun binary renamed (69MB), no separate "bun"
  // Must run as: amp run --cwd ... main.js login (same as bun)
  try {
    ...
    // Try "bun" first (some installs), then "amp" (Mac default = bun renamed)
    for (const name of ["bun", "amp"]) {
      const binPath = path.join(ampHome, "bin", name);
      if (fs.existsSync(binPath)) {
        console.log(`[amp-cli] resolved binary: ${binPath}`);
        return { bunPath: binPath, mainJs, pkgDir, ampHome };
      }
    }
  } catch { /* fallback */ }
  return null;
  ...
  function start(sessionId) {
    ...
    function pushChunk(chunk) {
      ...
      if (maybeUrl && isLoginUrl(maybeUrl)) {
        ...
        // Don't open browser here — amp CLI (bun direct) opens it automatically
      }
